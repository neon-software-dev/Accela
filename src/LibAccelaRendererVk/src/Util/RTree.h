...
#ifndef LIBACCELARENDERERVK_SRC_UTIL_RTREE_H
#define LIBACCELARENDERERVK_SRC_UTIL_RTREE_H
...
// RTree.h
//
// Taken from https://github.com/nushoin/RTree , version 1.0
//
// LICENSE: Public domain
//
// Modified further to work with Accela engine types (Volumes) and added additional search functionality
//
...
class RTree
{
    ...
    public:
        ...
        void Insert(const Volume& volume, const DATATYPE& a_dataId);
        ...
        void Remove(const Volume& volume, const DATATYPE& a_dataId);
        ...
        std::vector<DATATYPE> FetchMatching(const Volume& volume) const;
        std::vector<DATATYPE> FetchMatching(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const;
    ...
    protected:
        ...
        void FetchMatching(Node *a_node, Rect *a_rect, std::vector<DATATYPE>& found) const;
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert(const Volume& volume, const DATATYPE& a_dataId)
{
    float min[3] = {volume.min.x, volume.min.y, volume.min.z};
    float max[3] = {volume.max.x, volume.max.y, volume.max.z};

    Insert(min, max, a_dataId);
}
...
RTREE_TEMPLATE
void RTREE_QUAL::Remove(const Volume& volume, const DATATYPE& a_dataId)
{
    float min[3] = {volume.min.x, volume.min.y, volume.min.z};
    float max[3] = {volume.max.x, volume.max.y, volume.max.z};

    Remove(min, max, a_dataId);
}
...
RTREE_TEMPLATE
std::vector<DATATYPE> RTREE_QUAL::FetchMatching(const Volume& volume) const
{
    float min[3] = {volume.min.x, volume.min.y, volume.min.z};
    float max[3] = {volume.max.x, volume.max.y, volume.max.z};

    return FetchMatching(min, max);
}

RTREE_TEMPLATE
std::vector<DATATYPE> RTREE_QUAL::FetchMatching(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const
{
#ifdef _DEBUG
    for(int index=0; index<NUMDIMS; ++index)
  {
    ...
    toProcess.push(m_root);

    while (!toProcess.empty())
    {
        Node* a_node = toProcess.front();
        toProcess.pop();
        ...
        if (a_node->IsInternalNode())
        {
            ...
            for (int index = 0; index < a_node->m_count; ++index)
            {
                ...
                {
                    toProcess.push(a_node->m_branch[index].m_child);
                }
            }
        }
        else
...
RTREE_TEMPLATE
void
RTREE_QUAL::FetchMatching(Node *a_node, Rect *a_rect, std::vector<DATATYPE>& found) const
{
    assert(a_node);
    assert(a_node->m_level >= 0);
    assert(a_rect);
    ...
    if (a_node->IsInternalNode())
    {
        ...
        for (int index = 0; index < a_node->m_count; ++index)
        {
            if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
            {
                FetchMatching(a_node->m_branch[index].m_child, a_rect, found);
            }
        }
    }
    else
