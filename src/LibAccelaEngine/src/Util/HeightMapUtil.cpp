...
static glm::vec2 ModelPointToDataPoint(const LoadedHeightMap& heightMap, const glm::vec2& modelSpacePoint)
{
    ...
    const float x = MapValue({-halfModelWidth, halfModelWidth}, {0, heightMap.dataWidth - 1}, modelSpacePoint.x);
    const float y = MapValue({-halfModelHeight, halfModelHeight}, {0, heightMap.dataHeight - 1}, modelSpacePoint.y);
...
glm::vec2 DataRowColToModelPoint(const LoadedHeightMap& heightMap, const std::pair<unsigned int, unsigned>& rowCol)
{
    ...
    const float x = MapValue({0, heightMap.dataWidth - 1}, {-halfModelWidth, halfModelWidth}, (float)rowCol.first);
    const float y = MapValue({0, heightMap.dataHeight - 1}, {-halfModelHeight, halfModelHeight}, (float)rowCol.second);
